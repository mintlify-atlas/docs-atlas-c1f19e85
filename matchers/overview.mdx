---
title: "Built-in Matchers: Complete Reference Overview"
...
<CardGroup cols={2}>
  ...
  <Card title="Type Checks" icon="magnifying-glass" href="/matchers/type-checks">
    `toBeDefined`, `toBeNull`, `toBeString`, `toBeNumber`, `toBeArray`, and more.
  </Card>
...
| `toBeDefined()` | Type | Value is not `undefined` |
| `toBeUndefined()` | Type | Value is strictly `undefined` |
| `toBeNull()` | Type | Value is strictly `null` |
| `toBeNumber()` | Type | Value is a `number` and not `NaN` |
| `toBeNaN()` | Type | Value is `NaN` |
| `toBeString()` | Type | Value is a `string` |
| `toBeBoolean()` | Type | Value is a `boolean` |
| `toBeFunction()` | Type | Value is a `function` |
| `toBeArray()` | Type | Value is an `Array` |
| `toBeInstanceOf(ctor)` | Type | Value is an instance of the given constructor |
| `toBeTrue()` | Truthiness | Value is strictly `true` |
| `toBeFalse()` | Truthiness | Value is strictly `false` |
| `toBeTruthy()` | Truthiness | Value is truthy (`!!value === true`) |
| `toBeFalsy()` | Truthiness | Value is falsy (`!value === true`) |
...
| `toThrow()` | Error | Function throws when called |
...
| `not()` | Negation | Inverts the next matcher assertion |
...
`not()` returns the same `Expect<T>` instance with its internal `inverted` flag toggled, so the chain continues normally. See the [Negation](/matchers/negation) page for the full reference.
