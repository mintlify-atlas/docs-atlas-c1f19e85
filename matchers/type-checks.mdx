---
title: "Type, Existence, Truthiness, and Error Assertion Matchers"
sidebarTitle: "Type Checks"
description: "Assert value existence, JavaScript types, truthiness, and error throwing. Covers toBeDefined, toBeNull, toBeString, toBeTruthy, toBeTrue, and toThrow."
---
...
Use `not().toThrow()` to assert that a function completes **without** throwing. See the [Negation](/matchers/negation) page for detailed examples.
...
it('toBeTrue passes with strict true', () => {
    expect(true).toBeTrue();
    expect(1 === 1).toBeTrue();
});

it('toBeFalse passes with strict false', () => {
    expect(false).toBeFalse();
    expect(1 !== 1).toBeFalse();
});
```
...
it('toThrow fails with non-function values', () => {
    expect(() => expect(42).toThrow()).toThrow();
    expect(() => expect(null).toThrow()).toThrow();
});
```
