---
title: "expect(), Expect Class, and Matcher Interface API"
sidebarTitle: "expect"
...
```typescript
const extendMatchers: typeof createExpect;
```

A re-exported alias for `createExpect`. The two names are interchangeable; `extendMatchers` is provided as a more descriptive name when the intent is to add custom matchers to an assertion.
